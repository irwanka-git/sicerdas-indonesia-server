...
<table class="table table-striped table-sm table-bordered table-x">
	<thead>
		<tr class="table-primary">
			<th width="45%" class="text-center sm">PEMINATAN MAN</th>
	...
	<tbody>
		...
		$list = DB::select("SELECT * FROM ref_pilihan_minat_man");
		?>
		@foreach($list as $r)
		<tr>
			<td class="jf">
			...
			$klasifikasi = get_skor_predikat($data_skoring->$field_skoring,'skor','klasifikasi','ref_klasifikasi_minat_man');
			?>
			@foreach($list_klasifikasi as $l)
