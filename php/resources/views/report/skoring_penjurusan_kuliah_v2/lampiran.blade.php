...
<br>
<center>
	<b>DESKRIPSI 12 POLA GAYA PEKERJAAN</b>
</center>
...
<table class="table table-striped table-sm table-bordered table-x">
   <thead>
      <tr class="table-primary">
         <th width="15%" class="text-center sm">GAYA</th>
         <th width="30%" class="text-center sm">DESKRIPSI</th>
         <th width="55%" class="text-center sm">PEKERJAAN</th> 
      </tr>
   </thead>
   <tbody>
   @foreach ($gaya_pekerjaan as $l)
        <tr>
            <td align="center" class="sm">
                R{{$l->rangking}} <br> {!! $l->cetak_komponen !!} <br> ({!! $l->kode!!})
            </td>
            <td class="sm jf"> {{$l->deskripsi}}  </td>
            <td class="sm jf"> {{$l->pekerjaan}}  </td> 
        <tr>
    @endforeach
   </tbody>
</table>
