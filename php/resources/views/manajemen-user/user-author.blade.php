			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title">Pengaturan User Author</h5>
						<h6 class="card-subtitle text-muted">Fitur Ini Digunakan Untuk Manajemen User Author Pada Sistem </h6>
					</div>
					<div class="card-body">
						@if(ucc())
				   		{{Html::btnModal('<i class="la la-plus-circle"></i> Tambah Author','modal-tambah','primary')}}
				   		<hr>
				   		@endif
...
{{ Form::bsOpen('form-tambah',url($main_path."/insert")) }}
	{{Html::mOpenLG('modal-tambah','Tambah Author')}}
		{{ Form::bsTextField('Nama Author','nama_pengguna','',true,'md-8') }}
...
{{ Form::bsOpen('form-edit',url($main_path."/update")) }}
	{{Html::mOpenLG('modal-edit','Edit Author')}}
		{{ Form::bsTextField('Nama Author','nama_pengguna','',true,'md-8') }}
		{{ Form::bsTextField('Username','username','',true,'md-8') }}		
		{{ Form::bsTextField('Email','email','',true,'md-8') }}
		{{ Form::bsTextField('Telp','telp','',true,'md-8') }}
...
{{ Form::bsOpen('form-password',url($main_path."/update_password")) }}
	{{Html::mOpenLG('modal-password','Ubah Password Author')}}
		{{ Form::bsReadOnly('Nama Author','nama_pengguna','',true,'md-8') }}
...
<script type="text/javascript">
	$(function(){		 
		...
		var $tabel1 = $('#datatable').DataTable({
		    ...
		    fixedHeader: true,
		    serverSide: true,
		    ajax: "{{url('manajemen-user-author/dt')}}",
		    "iDisplayLength": 25,
		    columns: [
		...
		$("#modal-edit").on('show.bs.modal', function(e){
			...
			$.get("{{url('manajemen-user-author/get-data')}}/"+$uuid, function(respon){
				if(respon.status){
