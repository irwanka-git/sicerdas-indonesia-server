			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title">Pengaturan Komponen Karakteristik Pribadi </h5>
						<h6 class="card-subtitle text-muted">Fitur Ini Digunakan Untuk Input Data Komponen Karakteristik Pribadi</h6>
					</div>
					<div class="card-body">
						@if(ucc())
				   		{{Html::btnModal('<i class="la la-plus-circle"></i> Tambah Komponen Karakteristik','modal-tambah','primary')}}
				   		<hr>
				   		@endif
...
{{ Form::bsOpen('form-tambah',url($main_path."/insert")) }}
	{{Html::mOpenLG('modal-tambah','Tambah Komponen Karakter')}}
		{{ Form::bsTextArea('Nama Komponen','nama_komponen','',true,'md-8') }}
...
{{ Form::bsOpen('form-edit',url($main_path."/update")) }}
	{{Html::mOpenLG('modal-edit','Edit Komponen Karakter')}}
		{{ Form::bsTextArea('Nama Komponen','nama_komponen','',true,'md-8') }}
		{{ Form::bsTextArea('Keterangan','keterangan','',true,'md-8') }}
		{{ Form::bsHidden('uuid','') }}
	{{Html::mCloseSubmitLG('Simpan')}}
...
<script type="text/javascript">
	...
	var $tabel1 = $('#datatable').DataTable({
		    ...
		    fixedHeader: true,
		    serverSide: true,
		    ajax: "{{url('komponen-karakteristik-pribadi/dt')}}",
		    "iDisplayLength": 25,
		    columns: [
		    	 {data:'DT_Row_Index' , orderable:false, searchable: false,sClass:""},
		         {data:'nama_komponen' , name:"nama_komponen" , orderable:false, searchable: false,sClass:""},
		...
		$("#modal-edit").on('show.bs.modal', function(e){
			...
			$.get("{{url('komponen-karakteristik-pribadi/get-data')}}/"+$uuid, function(respon){
				if(respon.status){
					$('#form-edit #nama_komponen').val(respon.data.nama_komponen);
