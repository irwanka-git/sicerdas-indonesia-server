			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title">Skala Peminatan - SMK</h5>
						<h6 class="card-subtitle text-muted">Fitur Ini Digunakan Untuk Manajemen Kegiatan / Pernyataan Tentang Skala Peminatan SMK </h6>
						<p>Kode Warna: <b>#F3852C</b></p>
					</div>
					<div class="card-body">
						@if(ucc())
...
{{ Form::bsOpen('form-tambah',url($main_path."/insert")) }}
	{{Html::mOpenLG('modal-tambah','Tambah Kegiatan')}}
		...
		{{ Form::bsTextField('Paket','paket','',true,'md-8') }}
...
<script type="text/javascript">
	$(function(){
			...
			var $tabel1 = $('#datatable').DataTable({
			    ...
			    fixedHeader: true,
			    serverSide: true,
			    ajax: "{{url('soal-skala-peminatan-smk/dt')}}",
			    "iDisplayLength": 25,
			    columns: [
			    	 {data:'DT_Row_Index' , orderable:false, searchable: false,sClass:""},
			    	 {data:'paket' , name:"paket" , orderable:false, searchable: false,sClass:"text-center"},
			...
			//handle lihat soal
			 $("#modal-lihat-soal").on('show.bs.modal', function(e){
				...
				$.get("{{url('soal-skala-peminatan-smk/lihat-soal')}}/"+$uuid, function(respon){
		...
		$("#modal-edit").on('show.bs.modal', function(e){
			...
			$.get("{{url('soal-skala-peminatan-smk/get-data')}}/"+$uuid, function(respon){
				if(respon.status){
