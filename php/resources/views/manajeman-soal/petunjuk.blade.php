			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title">Pengaturan Petunjuk Soal</h5>
						<h6 class="card-subtitle text-muted">Fitur Ini Digunakan Untuk Input Data Petunjuk Soal</h6>
					</div>
					<div class="card-body">
						@if(ucc())
				   		{{Html::btnModal('<i class="la la-plus-circle"></i> Tambah Petunjuk Soal','modal-tambah','primary')}}
				   		<hr>
				   		@endif
...
{{ Form::bsOpen('form-tambah',url($main_path."/insert")) }}
	{{Html::mOpenLG('modal-tambah','Tambah Petunjuk Soal')}}
		{{ Form::bsTextArea('Isi Petunjuk','isi_petunjuk','',true,'md-8') }}
...
<script type="text/javascript">
	...
	var $tabel1 = $('#datatable').DataTable({
		    ...
		    fixedHeader: true,
		    serverSide: true,
		    ajax: "{{url('petunjuk-soal/dt')}}",
		    "iDisplayLength": 25,
		    columns: [
		    	 {data:'DT_Row_Index' , orderable:false, searchable: false,sClass:""},
		         {data:'isi_petunjuk' , name:"nama_menu" , orderable:false, searchable: false,sClass:""},
		...
		$("#modal-edit").on('show.bs.modal', function(e){
			...
			$.get("{{url('petunjuk-soal/get-data')}}/"+$uuid, function(respon){
				if(respon.status){
					$('#form-edit #isi_petunjuk').val(respon.data.isi_petunjuk);
