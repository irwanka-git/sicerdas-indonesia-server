...
loadHelper('akses'); 
$list_komponen = DB::table('ref_komponen_kepribadian_manajerial')
					->select('id as value','komponen as text')->orderby('id','asc')->get();
?>
@extends('layout')
@section("css")
			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title">Soal Kepribadian Manajerial</h5>
						<h6 class="card-subtitle text-muted">Fitur Ini Digunakan Untuk Manajemen Soal / Pernyataan Tentang Kepribadian Manajerial </h6>
					</div>
					<div class="card-body">
						<p>Pilihan: 1. STS (Sangat Tidak Sesuai), 2. TS (Tidak Sesuai), 3. N (Netral), 4. S (Sesuai) , 5. SS (Sesuai Sekali)</p>
						@if(ucc())
...
<script type="text/javascript">
	$(function(){
			...
			var $tabel1 = $('#datatable').DataTable({
			    ...
			    fixedHeader: true,
			    serverSide: true,
			    ajax: "{{url('soal-kepribadian-manajerial/dt')}}",
			    "iDisplayLength": 25,
			    columns: [
			...
			//handle lihat soal
			 $("#modal-lihat-soal").on('show.bs.modal', function(e){
				...
				$.get("{{url('soal-kepribadian-manajerial/lihat-soal')}}/"+$uuid, function(respon){
		...
		$("#modal-edit").on('show.bs.modal', function(e){
			...
			$.get("{{url('soal-kepribadian-manajerial/get-data')}}/"+$uuid, function(respon){
				if(respon.status){
