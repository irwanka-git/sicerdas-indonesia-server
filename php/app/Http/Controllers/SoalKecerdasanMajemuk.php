...
class SoalKecerdasanMajemuk extends Controller
{
    //
    function index(){
        $pagetitle = "Soal Skala Kecerdasan Majemuk";
        $smalltitle = "Pengaturan Soal";
        return view('manajeman-soal.index-kecerdasan-majemuk', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* from soal_kecerdasan_majemuk as a where a.nomor <> '' $filter  ";
    ...
    function submit_insert(Request $r){
        ...
        if($this->ucc()){
            ...
            DB::table('soal_kecerdasan_majemuk')->insert($record);
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_kecerdasan_majemuk')->where('uuid', $uuid)->first();
        if($soal){
    ...
    function get_data($uuid){
        $data = DB::table('soal_kecerdasan_majemuk')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_update(Request $r){
        if($this->ucu()){
            ...
            DB::table('soal_kecerdasan_majemuk')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_kecerdasan_majemuk')->where('uuid', $uuid)->delete();
