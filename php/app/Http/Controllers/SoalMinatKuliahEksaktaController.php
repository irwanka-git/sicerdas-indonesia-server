...
class SoalMinatKuliahEksaktaController   extends Controller
{
    ...
    //######################### SOAL TEST KOGNITIF #####################################
    function index(){
    	$pagetitle = "Soal Minat Kuliah Eksakta";
    	$smalltitle = "Pengaturan Soal";
    	return view('manajeman-soal.index-mk-eksakta', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* , b.kelompok, c.kelas from soal_minat_kuliah_eksakta as a left join 
                        ref_kelompok_minat_kuliah as b on a.id_kelompok = b.id
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_minat_kuliah_eksakta')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajeman-soal.view-soal-mk-eksakta',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
    	$data = DB::table('soal_minat_kuliah_eksakta')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_minat_kuliah_eksakta')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_minat_kuliah_eksakta')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_minat_kuliah_eksakta')->where('uuid', $uuid)->delete();
