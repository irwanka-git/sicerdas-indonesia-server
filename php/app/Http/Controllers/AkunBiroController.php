...
class AkunBiroController extends Controller
{
     private $id_role=5;

      function index(){
    	$pagetitle = "Pengaturan Akun Biro";
    	$smalltitle = "Pengaturan Akun Biro Pelaksana Tes";
    	return view('manajemen-user.akun-biro', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(){
        ...
        ->addColumn('action', function ($query) {
                ...
                if($this->ucu()){
                    ...
                    $edit = '<button data-bs-tip="tooltip" data-bs-placement="top" data-bs-original-title="Ubah Password" data-bs-toggle="modal" data-uuid="'.$query->uuid.'" data-bs-target="#modal-password" class="btn btn-light btn-sm" type="button">
                    <i class="las la-key"></i></button>';
                    
                    $edit .= ' <button data-bs-tip="tooltip" data-bs-placement="top" data-bs-original-title="Ubah Akun"  data-bs-toggle="modal" data-uuid="'.$query->uuid.'" data-bs-target="#modal-edit" class="btn btn-light btn-sm" type="button">
                    <i class="las la-pen"></i></button>';
                }
                if($this->ucd()){
                    $delete = '<button  data-bs-tip="tooltip" data-bs-placement="top" data-bs-original-title="Hapus Akun"  data-uuid="'.$query->uuid.'" class="btn btn-light btn-sm btn-konfirm-delete" type="button">
                    <i class="las la-trash"></i></button>';
                }
                $action = $edit." ".$delete;
                
                if ($action==" "){
                	$action='<a href="#" class="act"><i class="la la-lock"></i></a>'; 
                }else{
                	$action =   '<div class="btn-group" role="group">'.$action.'</button>';
                }
                return $action;
        })

        ->editColumn('nama_pengguna', function ($query) {
                
                if($this->ucu()){
                        $url = url('manajemen-akun-biro/profil/'.$query->uuid);
                     return ' <a data-bs-tip="tooltip" data-bs-placement="top" data-bs-original-title="Ubah Profil Biro"   href="'.$url.'">'.$query->nama_pengguna.'</a>';
                }
                
                return $query->nama_pengguna;
        })
        ->addIndexColumn()
        ->rawColumns(['action','menu','nama_pengguna'])
        ->make(true);
    }
