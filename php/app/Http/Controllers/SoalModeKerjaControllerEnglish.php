...
class SoalModeKerjaControllerEnglish  extends Controller
{
    ...
    function index(){
    	...
    	return view('manajemen-soal-eng.index-soal-mode-kerja', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select * from soal_mode_kerja_eng where urutan > 0 $filter order by urutan";
    ...
    function get_data($uuid){
    	$data = DB::table('soal_mode_kerja_eng')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_mode_kerja_eng')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_mode_kerja_eng')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_mode_kerja_eng')->where('uuid', $uuid)->delete();
