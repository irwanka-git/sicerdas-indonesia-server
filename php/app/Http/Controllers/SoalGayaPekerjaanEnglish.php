...
class SoalGayaPekerjaanEnglish extends Controller
{
    //
    function index(){
        $pagetitle = "Soal Skala Gaya Pekerjaan";
        $smalltitle = "Pengaturan Soal";
        return view('manajemen-soal-eng.index-gaya-pekerjaan', compact('pagetitle','smalltitle'));
    }

    function datatable(Request $r){

        $ref_komponen = DB::table('ref_komponen_gaya_pekerjaan_eng')->orderby('no')->get();
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* from soal_gaya_pekerjaan_eng as a where a.nomor > 0 $filter  ";
    ...
    function submit_insert(Request $r){
        ...
        if($this->ucc()){
            ...
            DB::table('soal_gaya_pekerjaan_eng')->insert($record);
    ...
    function get_data($uuid){
        $data = DB::table('soal_gaya_pekerjaan_eng')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_update(Request $r){
        if($this->ucu()){
            ...
            DB::table('soal_gaya_pekerjaan_eng')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_gaya_pekerjaan_eng')->where('uuid', $uuid)->delete();
