...
class SoalSikapPelajaranKuliahController   extends Controller
{
    ...
    function index(){
    	...
    	return view('manajeman-soal.index-sikap-pelajaran-kuliah', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* from soal_sikap_pelajaran_kuliah as a where a.id_soal > 0 $filter  ";
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_sikap_pelajaran_kuliah')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajeman-soal.view-soal-sikap-pelajaran-kuliah',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
    	$data = DB::table('soal_sikap_pelajaran_kuliah')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_sikap_pelajaran_kuliah')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_sikap_pelajaran_kuliah')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_sikap_pelajaran_kuliah')->where('uuid', $uuid)->delete();
