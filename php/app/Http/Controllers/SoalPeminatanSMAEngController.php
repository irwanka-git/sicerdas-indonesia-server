...
class SoalPeminatanSMAEngController  extends Controller
{
     
    //######################### SOAL TEST KOGNITIF #####################################
    function index(){
    	$pagetitle = "Soal Skala Peminatan SMA ENG";
    	$smalltitle = "Pengaturan Soal";
    	return view('manajemen-soal-eng.index-peminatan-sma', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* from soal_peminatan_sma_eng as a where a.id_soal > 0 $filter  order by urutan asc ";
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_peminatan_sma_eng')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajemen-soal-eng.view-soal-peminatan-sma',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
    	$data = DB::table('soal_peminatan_sma_eng')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_peminatan_sma_eng')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_peminatan_sma_eng')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_peminatan_sma_eng')->where('uuid', $uuid)->delete();
