...
class PetunjukSoalControllerEnglish extends Controller
{
    ...
    function index(){
    	...
    	return view('manajemen-soal-eng.petunjuk', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.isi_petunjuk , a.id_petunjuk, a.uuid  
         	from petunjuk_soal_eng as a where  a.id_petunjuk > 0  $filter 
    ...
    function get_data($uuid){
    	$data = DB::table('petunjuk_soal_eng')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('petunjuk_soal_eng')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('petunjuk_soal_eng')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('petunjuk_soal_eng')->where('uuid', $uuid)->delete();
