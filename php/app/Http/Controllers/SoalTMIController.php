...
class SoalTMIController   extends Controller
{
     
    //######################### SOAL TEST KOGNITIF #####################################
    function index(){
    	$pagetitle = "Soal Tes Minat Indonesia";
    	$smalltitle = "Pengaturan Soal";
    	return view('manajeman-soal.index-tmi', compact('pagetitle','smalltitle'));
    }
        ...
        if (request()->has('search')) {
            ...
            if(strlen($keyword)>=3){
                ...
                $filter = " and ( lower(a.pernyataan) like '%$keyword%'  
                			or  lower(a.minat) like '%$keyword%'                       
                            ) ";
            }   
        }

         $sql_union = "select a.* from soal_tmi as a where a.id_soal > 0 $filter  ";
         ...
         return Datatables::of($query)
            ...
            ->editColumn('kelompok', function ($q){
            	return $q->kelompok."<br><small>".$q->minat."</small>";
            })->editColumn('pernyataan', function ($q){
            	return "<small>".$q->pernyataan."</small>";
            })
            ->rawColumns(['action','pernyataan','kelompok'])
            ->make(true);
    }
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_tmi')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajeman-soal.view-soal-tmi',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
    	$data = DB::table('soal_tmi')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_tmi')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_tmi')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_tmi')->where('uuid', $uuid)->delete();
