...
class SoalMinatIlmuAlamEngController   extends Controller
{
     
    function index(){
    	$pagetitle = "Soal Minat Ilmu Alam (ENG)";
    	$smalltitle = "Pengaturan Soal";
    	return view('manajemen-soal-eng.index-ilmu-alam', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* , b.kelompok, c.kelas from soal_minat_kuliah_eksakta_eng as a left join 
                        ref_kelompok_minat_kuliah_eng as b on a.id_kelompok = b.id
                        left join ref_kelas_minat_sma_eng as c on a.id_kelas = c.id
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_minat_kuliah_eksakta_eng')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajemen-soal-eng.view-soal-ilmu-alam',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
    	$data = DB::table('soal_minat_kuliah_eksakta_eng')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_minat_kuliah_eksakta_eng')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_minat_kuliah_eksakta_eng')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_minat_kuliah_eksakta_eng')->where('uuid', $uuid)->delete();
