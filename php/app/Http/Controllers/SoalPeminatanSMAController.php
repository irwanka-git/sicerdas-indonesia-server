...
class SoalPeminatanSMAController  extends Controller
{
     
    //######################### SOAL TEST KOGNITIF #####################################
    function index(){
    	$pagetitle = "Soal Skala Peminatan SMA";
    	$smalltitle = "Pengaturan Soal";
    	return view('manajeman-soal.index-peminatan-sma', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* from soal_peminatan_sma as a where a.id_soal > 0 $filter  ";
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_peminatan_sma')->where('uuid', $uuid)->first();
        if($soal){
    ...
    function get_data($uuid){
    	$data = DB::table('soal_peminatan_sma')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	DB::table('soal_peminatan_sma')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_peminatan_sma')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_peminatan_sma')->where('uuid', $uuid)->delete();
