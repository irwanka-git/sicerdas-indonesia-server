...
class ApiUserController  extends ApiController
{
	...
	function get_info_contact(){
		...
		$respon = array('status'=>false,'data'=>null);
		return $this->respon_json($respon);
	}
	...
	function submit_login(Request $r){
		...
		$username = $r->username;
		$password = $r->password;
		$device_id = trim($r->device_id);

		$cek_user = DB::table('users')->where('username', $username)->first();
		if($cek_user){
			if($cek_user->device_id!= "" && $device_id !="" ){
				$respon = array('status'=>false,'message'=>"Akses ditolak karena terdapat aktivitas Login di perangkat lain. Silahkan Logout akun anda pada perangkat lain. Untuk Informasi Lebih Lanjut Hubungi Administrator Sicerdas");
        		//return $this->respon_json($respon);	
			}
		}
		...
		if(Auth::attempt(['username' => $username, 'password' => $password])){
			...
			//$array = DB::table('soal_peminatan_smk')->get();
			$respon = array('status'=>true,'message'=>'Hai '.$user->nama_pengguna.', Selamat Datang di Si Cerdas Indonesia!', 'token'=>$token, 'data'=>$user_data,);
        	return $this->respon_json($respon);	
		}else{
			$respon = array('status'=>false,'message'=>"Username dan Password Tidak Valid");
        	return $this->respon_json($respon);	
	...
	function logout(){
		...
		if($user){
			DB::table('users')
				->where('id', $user->id)
				->update(['device_id'=>'']);
			$respon = array('status'=>true,'message'=>'Berhasil Logout dari Aplikasi!');
			return $this->respon_json($respon);	
		}else{
