...
class SoalKecerdasanMajemukEng extends Controller
{
    //
    function index(){
        $pagetitle = "Soal Skala Kecerdasan Majemuk ENG";
        $smalltitle = "Pengaturan Soal";
        return view('manajemen-soal-eng.index-kecerdasan-majemuk', compact('pagetitle','smalltitle'));
    }
    ...
    function datatable(Request $r){
        ...
        if (request()->has('search')) {
         ...
         $sql_union = "select a.* from soal_kecerdasan_majemuk_eng as a where a.nomor <> '' $filter  ";
    ...
    function submit_insert(Request $r){
        ...
        if($this->ucc()){
            ...
            DB::table('soal_kecerdasan_majemuk_eng')->insert($record);
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_kecerdasan_majemuk_eng')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajemen-soal-eng.view-soal-kecerdasan-majemuk',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
        $data = DB::table('soal_kecerdasan_majemuk_eng')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_update(Request $r){
        if($this->ucu()){
            ...
            DB::table('soal_kecerdasan_majemuk_eng')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_kecerdasan_majemuk_eng')->where('uuid', $uuid)->delete();
