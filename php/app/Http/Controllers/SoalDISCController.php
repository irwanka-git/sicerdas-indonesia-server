...
class SoalDISCController  extends Controller
{
    ...
    function index(){
    	...
    	return view('manajeman-soal.index-disc', compact('pagetitle','smalltitle'));
    }
        ...
        if (request()->has('search')) {
            ...
            if(strlen($keyword)>=3){
                ...
                $filter = " and ( lower(a.pernyataan) like '%$keyword%' 
                            ...
                            or lower(a.pilihan_i) like '%$keyword%'
                            or lower(a.pilihan_s) like '%$keyword%'                           
                            or lower(a.pilihan_c) like '%$keyword%'                           
                            ) ";
            }   
        }

         $sql_union = "select a.* from soal_disc as a where a.id_soal > 0 $filter  ";
         ...
         return Datatables::of($query)
            ...
            ->addColumn('pilihan', function ($q) {
                $pilihan = "<ul>";
                $pilihan_d = "<li>(D) ".$q->pilihan_d . "</li>";
                $pilihan_i = "<li>(I) ".$q->pilihan_i. "</li>";
                $pilihan_s = "<li>(S) ".$q->pilihan_s . "</li>";
                $pilihan_c = "<li>(C) ".$q->pilihan_c . "</li>";
                return "<small>".$pilihan.$pilihan_d.$pilihan_i.$pilihan_s.$pilihan_c."</ul></small>";
            })
            ->addIndexColumn()
    ...
    function view_soal($uuid){
        $soal = DB::table('soal_disc')->where('uuid', $uuid)->first();
        if($soal){
            return view('manajeman-soal.view-soal-peminatan-sma',compact('soal'));
        }
        return '<center><b>Maaf, Soal Tidak Ditemukan</b></center>';
    }

    function get_data($uuid){
    	$data = DB::table('soal_disc')->where('uuid', $uuid)->first();
        if($data){
    ...
    function submit_insert(Request $r){
    	if($this->ucc()){
	    	...
	    	$record = array(
                ...
                "pilihan_i"=>trim($r->pilihan_i), 
                "pilihan_s"=>trim($r->pilihan_s), 
                "pilihan_c"=>trim($r->pilihan_c), 
	    		"uuid"=>$uuid);

	    	DB::table('soal_disc')->insert($record);
    ...
    function submit_update(Request $r){
    	if($this->ucu()){
	    	...
	    	DB::table('soal_disc')->where('uuid', $uuid)->update($record);
    ...
    function submit_delete(Request $r){
        if($this->ucd()){
            loadHelper('format');
            $uuid = $r->uuid;
            DB::table('soal_disc')->where('uuid', $uuid)->delete();
